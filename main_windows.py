...
def ecbEncryptedPattern(filename, outputFileName, format):
    ...
    new = convert_to_RGB(aes_ecb_encrypt(key, data)[:original])
...
def encryptAndDecryptWithAllModes(path):
    ...
    ecbEncryptedPattern(path, saveEncryptionPath + "ecb_pattern_" + encryptedName, name[-1])
